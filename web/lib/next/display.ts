...
/** Listing ids in kind order, formatted for the quiet ID stack. */
...
export function dealIdLines(deal: DisplayDeal): string[] {
  ...
  const lines = tly ? [tly] : [];
  for (const label of listingIdLabels(deal)) {
    if (label && !lines.includes(label)) lines.push(label);
  }
  return lines;
}

/** Joined form for facts tables / tests. */
export function dealIdLine(deal: DisplayDeal): string {
  return dealIdLines(deal).join(" · ");
}
