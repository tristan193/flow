...
| Dealbook | `deals_next` has vote/note/watch columns (backfilled from child tables) plus empty `deal_log` (going forward only). Visual home: `/db`. Review/Pipeline still read `verdicts_next` / `notes_next` / etc. until a later cutover. |
